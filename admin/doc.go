...
//   - the migrator the consumer built over its migration set
//   - the pattern catalog every statement compiles against
//
// [Options] carries the optional collaborators: a [Seeder], a [Registry], a
// logger, and the name of the set that applies at startup. A nil required
// collaborator, or a startup set without a seeder, is a wiring defect and
// panics, per the database package's wiring rule.
//
// # Startup
//
...
//
//   - [Service.Verify], [Service.Status], [Service.Up], [Service.Down],
//     [Service.Steps], and [Service.Force] call the migrator's verb of the
//     same name; the mutating verbs return the refreshed [Status].
//   - [Service.States] lists the seeder's declared names.
//   - [Service.Seed] applies a named set, or the configured one, over the
//     schema as it stands.
//   - [Service.Reset] reverts every applied migration, applies the whole
//     set, and seeds the named state's set — the one transition that
//     brings a database to a named state from any other.
...
// The HTTP half, a route group over these methods, is application code.
package admin
